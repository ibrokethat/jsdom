...
defineGetter(core.Node.prototype, 'dataset', function () {

  if (!this._dataset) {

    this._dataset = (function (el) {

      var data = {};

      for (var att, i = 0, atts = el.attributes, n = atts.length; i < n; i++){
        att = atts[i];
        if (/^data-/.test(att.nodeName)) {
          var name = att.nodeName.split('data-')[1];
          name = name.replace(/-([a-z])/g, function (g) {return g[1].toUpperCase();});
          data[name] = att.nodeValue;
        }
      }

      return Proxy.create({

        set: function (reciever, prop, val) {

          data[prop] = val;
          var name = 'data-' + prop.replace(/([a-z][A-Z])/g, function (g) {return g[0] + '-' + g[1].toLowerCase()});
          this.setAttribute(name, val);
        },

        get: function (reciever, prop) {
          return data[prop];
        }
